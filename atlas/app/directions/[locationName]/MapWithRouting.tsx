      ...
      <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4 shadow-lg border border-blue-100">
        ...
        <div className="space-y-2.5">
          <button
            ...
            className="w-full bg-[#7A96D5] text-white py-2.5 rounded-lg hover:bg-[#6A86C5] transition-all duration-200 font-medium shadow-sm hover:shadow"
          >
            Ground Floor
          </button>
          <button
        ...
        <button
          onClick={onClose}
          className="w-full mt-4 bg-gray-100 text-gray-600 py-2.5 rounded-lg hover:bg-gray-200 transition-all duration-200 font-medium"
        >
          Close
        </button>
      </div>
          ...
          <div className="relative aspect-video bg-gray-50 rounded-lg overflow-hidden border border-gray-200">
            {isLoading && !imageError && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#7A96D5]"></div>
              </div>
            )}
...
const DestinationMarker = ({ position, locationName, onViewFloorPlans }: {
  ...
  return (
    <Marker 
      ...
      eventHandlers={{
        popupclose: () => {
          // A small hack to prevent the popup from closing permanently
          setTimeout(() => {
            markerRef.current?.openPopup();
          }, 100);
        }
